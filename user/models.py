...
class User:
    def signup(self):
        ...
        user = {
            ...
            "da": request.form.get('da')
        }

        # Check for duplicate entries
        if db.check_for_user(user["email"], user["da"]):
            return False
        else:
            db.add_user(user)
            return True
    ...
    def login(self):
        ...
        user_exists = db.check_for_user(request.form.get('email'), request.form.get('da'))
    ...
    def da(self):
        if (request.form.get("da") == "Detective"):
            return request.form.get("name")
        elif (request.form.get("da") == "Admin"):
            return False
